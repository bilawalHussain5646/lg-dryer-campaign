...
var productType = ($('#resultProductList .cardlist-box').length > 0) ? 'card' : 'default';
	...
	if(productType == 'card') {
		...
		if(obj_ea * obj_width  > group_width){
			...
			if(!_this.find('button').hasClass('btn-limited')){
				_this.append('<button class="btn-limited">'+ '+ ' + (obj_ea-countItemViewport) + '</button>')
	...
	var pageHistory = {
		...
		setHistory: function(va) {
			...
			if(pageHistory.usePage) {
				...
				if(typeof ePrivacyCookies=='undefined' || ePrivacyCookies.get('LGCOM_IMPROVEMENTS') || COUNTRY_CODE.toLowerCase()=='de' || COUNTRY_CODE.toLowerCase()=='it'|| COUNTRY_CODE.toLowerCase()=='es') { //LGEDE-926,LGEIS-1124,LGEES-481
	...
	filter = {
				...
				if(typeof ePrivacyCookies=='undefined' || ePrivacyCookies.get('LGCOM_IMPROVEMENTS')) {
					if($(".GPC0007").length > 0 || $(".GPC0026").length > 0){ 
						...
						$(window).on('load scroll', function(){
							filterLine = parseInt($(".GPC0007, .GPC0026").find(".product-list-box").offset().top );
							var scrollPos =  parseInt($(window).scrollTop()) + parseInt(controlStickyElements.pos.FILTER);
						...
						$(".GPC0007 .filter-open-floating .floating-filter").on({
							click:function(e){
								...
								if($(this).attr("class").match("active")){
									...
									filterTopLine = parseInt( $(".GPC0007").offset().top ) - parseInt(controlStickyElements.pos.FILTER);
						...
						$(".GPC0026 .filter-open-floating .floating-filter").on({
							click:function(e){
								...
								if($(this).attr("class").match("active")){
									...
									$(".filter-box .filter-result").css("display","block");
									// floating-filter 클릭시 위치 조절
									filterTopLine = parseInt( $(".GPC0026").offset().top ) - parseInt(controlStickyElements.pos.FILTER);
									$(window).scrollTop(filterTopLine);
								}else{
									$(this).addClass("active");
		...
		slider: {
			...
			slide: function( event, ui ) {
				...
				if(typeof ePrivacyCookies=='undefined' || ePrivacyCookies.get('LGCOM_IMPROVEMENTS')||COUNTRY_CODE.toLowerCase()=='es'|| COUNTRY_CODE.toLowerCase()=='uk' || COUNTRY_CODE.toLowerCase()=='it') {//LGEIS-1124
		...
		cookie: {
			...
			bake: function(param){
				...
				if(_this.cont.join('') == "") {
					...
					/*LGEES-15 modify*/
					if(typeof ePrivacyCookies=='undefined' || ePrivacyCookies.get('LGCOM_IMPROVEMENTS') ||COUNTRY_CODE.toLowerCase()=='es' || COUNTRY_CODE.toLowerCase()=='de'|| COUNTRY_CODE.toLowerCase()=='it') { //LGEDE-926,LGEIS-1124
		...
		init: function(){
			...
			if($('[data-del-nofollow-url]').length){
				...
				if(delNofollowUrl != 'plp-del-nofollow-url' && delNofollowUrl.indexOf(location.pathname) > -1){
					filter.template = filter.template.replace(/rel=\"nofollow\"/g,'').replace(/\"nofollow\"=\"\"/g,'');
				}
			}
		...
		createProductItem: function(productList, productMessages, data){
			...
			for (var i = 0; i < productList.length; i++) {
				...
				template = template.replace(/\*modelId\*/g, p.modelId)
								...
								.replace(/\*productTag3UserType\*/g, p.productTag3UserType) /* PJTQUICKWIN */
								...
								// LGECZ-402, LGECZ-421 Start
								.replace(/\*lowestPriceToggleClass\*/g, p.obsLowestPriceFlag != 'Y' ? 'd-none' : '')
								.replace(/\*lowestPrice\*/g, p.obsLowestPriceFlag == 'Y' && p.obsLowestPrice > 0 ? changeFormatFullPrice(p.obsLowestPrice, p.obsLowestPriceCent) : '') // LGECZ-429
								.replace(/\*lowestPriceFlag\*/g, p.obsLowestPriceFlag)
								// LGECZ-402, LGECZ-421 End
				...
				var productFicheDownload = $('input[name="productFicheDownload"]').val();
			...
			template = template.replace(/\*secondEnergyLabelImage\*/g,secondEnergyLabelImagehtml);
				...
				if(p.obsLowestPriceFlag != 'Y') $template.find('.lowest-price').remove(); // LGECZ-424
				...
				for (var i1 = 0; i1 < $keyBlocks.length; i1++) {
					...
					if(!val || (val == null || val == "N")) {
						if($currentKeyBlock.is('.btn')) {
							...
							// PJTQUICKWIN
							if((key == 'productTag1' ||  key == 'productTag2'||  key == 'productTag3') && ($('.GPC0007').length>0 ||$('.GPC0026').length>0)) {
				...
				if($('.GPC0007').length>0 ||$('.GPC0026').length>0) {
					...
					if(productType != 'card') { // card type에서는 label 사용하지 않음
						...
						if(p.productTag1 != null || p.productTag1 != "" || p.productTag2 != null || p.productTag2 != "" || p.productTag3 != null || p.productTag3 != "") {
							ynProductTag = "Y";
						}
					}
				}
			...
			if(productType == 'card') {
				...
				if(p.addToCartFlag == 'Y') {
					...
					if(p.obsBuynowFlag == 'Y'){ // Buy now 버튼 사용하지 않음
						// 통합 OBS
						...
						// 통합 OBS
						$template.find('a.add-to-cart').removeClass('hidden').data('model-id', p.modelId).attr('href', '#').attr('role', 'button').removeAttr('target, title');
					}
				...
				if(p.installationFlag=="Y" && p.installationFlag != null && p.installationFlag != '') {
				...
				if(p.inquiryToBuyFlag=="Y" && p.inquiryToBuyUrl != null && p.inquiryToBuyUrl != '') {
					$template.find('a.inquiry-to-buy').removeClass('hidden').attr('href', p.inquiryToBuyUrl).text(productMessages.inquiryToBuyBtnNm);
				} else {
				...
				if(p.whereToBuyFlag=="Y" && p.whereToBuyUrl != null && p.whereToBuyUrl != '') {
					// go to pdp page
					$template.find('a.where-to-buy').attr('href', p.whereToBuyUrl).text(productMessages.whereToBuyBtnNm).parent().removeClass('hidden');
					$template.find('a.where-to-buy').parent().removeAttr('target, title');
					...
					// go to external link
					$template.find('a.in-buynow:not(.add-to-cart)').attr('href', p.wtbExternalLinkUrl).text(p.wtbExternalLinkName).attr('data-link-name', 'buy_now').removeAttr('data-sc-item').parent().removeClass('hidden');
					if(p.wtbExternalLinkSelfFlag == 'Y') {
						$template.find('a.in-buynow:not(.add-to-cart)').removeAttr('target, title');
					} else {
						$template.find('a.in-buynow:not(.add-to-cart)').attr('target', '_blank').attr('title', productMessages.btnNewLinkTitle);
					}
				} else {
				...
				if(p.findTheDealerFlag=="Y" && p.findTheDealerUrl != null && p.findTheDealerUrl != '') {
					$template.find('a.find-a-dealer').attr('href', p.findTheDealerUrl).text(productMessages.findTheDealerBtnNm).parent().removeClass('hidden');
				} else {
			...
			// PJTQUICKWIN
			if(productType == 'card') {
				$template.find('.area-top .tag-box').find('[data-user-type=""]').removeClass('d-none');
				$template.find('.area-top .tag-box').find('[data-user-type=ALL]').removeClass('d-none');
				if(SIGN_IN_STATUS == 'Y' && ISVIP){
					$template.find('.area-top .tag-box').find('[data-user-type=VIP]').removeClass('d-none');
				}else{
					$template.find('.area-top .tag-box').find('[data-user-type=NON_VIP]').removeClass('d-none');
				}
			} else {
				//LGEDE-354 start
		...
		reset: function(e){
			/*LGEES-15 modify*/
			if(typeof ePrivacyCookies=='undefined' || ePrivacyCookies.get('LGCOM_IMPROVEMENTS')||COUNTRY_CODE.toLowerCase()=='es' || COUNTRY_CODE.toLowerCase()=='uk' || COUNTRY_CODE.toLowerCase()=='de'|| COUNTRY_CODE.toLowerCase()=='it') { //LGEDE-926,LGEIS-1124
				var form = filter.el;
	...
	plp = {
		...
		addEvent: function(){
			...
			$(plp.el.list).off('click').on({ /* BTOBGLOBAL-434 : 20210414 modify */
				click: function(e){
					e.preventDefault();
					if(typeof ePrivacyCookies=='undefined' || ePrivacyCookies.get('LGCOM_IMPROVEMENTS')  ||COUNTRY_CODE.toLowerCase()=='uk' || COUNTRY_CODE.toLowerCase()=='de'|| COUNTRY_CODE.toLowerCase()=='it' || COUNTRY_CODE.toLowerCase()=='es') { //LGEDE-926,LGEIS-1124,LGEES-481
						var page;
		...
		filter2 = {
			...
			createProductItem: function(productList, productMessages){
				...
				for (var i = 0; i < productList.length; i++) {
					...
					if($('.GPC0134').length > 0 || $('.GPC0142').length > 0) {
						if(i == 0) {
							...
							//LGEFR-640
							$('#whiteSpaceCurData [data-repair-area-yn]').attr("data-repair-area-yn", productList[i].repairModelAreaYn);
						}
					}
					var p = productList[i],
					...
					template = template.replace(/\*modelId\*/g, p.modelId)
									...
									.replace(/\*afterPayInstallMent\*/g, p.obsEmiMsgFlag == 'Y' && p.emiMsg != null && p.emiMsg != '' && (p.afterPay <= 3000 && p.afterPay > 0) ? 'afterpay-installment" href="#modal-afterpay' : '" style="display:none;')/* LGEAU-378 add */
									.replace(/\*wtbClass\*/g,(p.wtbExternalLinkUseFlag =="Y" && p.wtbExternalLinkUrl != null && p.wtbExternalLinkUrl != '' && p.wtbExternalLinkName != null && p.wtbExternalLinkName != '') ? 'in-buynow' : 'where-to-buy') //LGEGMC-2202
									.replace(/\*membershipPrice\*/g, changeFormatFullPrice(p.rMembershipPrice, p.rMembershipPriceCent)) /* PJTMEMBERSHIP-11 */
									.replace(/\*membershipDisplayFlag\*/g, p.membershipDisplayFlag) /* PJTMEMBERSHIP-11 */
									.replace(/\*isHasMember\*/g,(hasMemberFlag =="Y") ? 'has-member' : '')  /* PJTMEMBERSHIP-11 */
									.replace(/\*obsLowestPrice\*/g, !!p.obsLowestPriceCent && p.obsLowestPriceCent != null ? changeFormatFullPrice(p.obsLowestPrice, p.obsLowestPriceCent) : changeFormatFullPrice(p.obsLowestPrice) ) // LGECZ-424
				...
				template = template.replace(/\*secondEnergyLabelImage\*/g,secondEnergyLabelImagehtml);
					...
					//LGEGMC-3675 START
					if(typeof p.pdrCompareUseFlag != 'undefined' && null != p.pdrCompareUseFlag) {
			...
			createProductItemCuration : function(productList, productMessages){
				...
				for (var i = 0; i < productList.length; i++) {
					var p = productList[i],
						template = $('.GPC0134 .result-sub-aria .GPC0004').find('.contents-template').clone().html();
					var priceValue = "";
					if(p.rPromoPrice != null && p.rPromoPrice != "" && p.rPromoPrice != 'null'){
						priceValue = p.rPromoPrice+"."+nvl(p.rPromoPriceCent,'00');
					} else{
					...
					// LGECZ-424 Start
					var obsLowestPrice = "";
					if(p.obsLowestPriceFlag == 'Y' && p.obsLowestPrice > 0 && p.obsLowestPriceCent != 'null'){
						obsLowestPrice = nvl(p.obsLowestPrice,'')+"."+nvl(p.obsLowestPriceCent,'00');
					}
					// LGECZ-424 End
					...
					recommendedRetailRriceInfo = $('.GPC0134 .result-sub-aria .GPC0004').data('recommended-retail-price-info'),
					rPrice = p.rPrice,
					...
					template = template.replace(/\*modelId\*/g, p.modelId)
									...
									.replace(/\*obsLowestPrice\*/g, obsLowestPrice) // LGECZ-424
									.replace(/\*obsPartnerUrl\*/g, nvl(p.obsPartnerUrl, '')) //LGEEG-154
									.replace(/\*zipPayInstallMent\*/g, p.obsEmiMsgFlag == 'Y' && (p.afterPay > 0 && p.afterPay <= 5000) ? 'afterpay-installment" href="#modal-afterpay' : '" style="display:none;')/* LGEGMC-3167 add */
									.replace(/\*orZipPay\*/g, p.obsEmiMsgFlag == 'Y' && (p.afterPay > 0 && p.afterPay <= 3000) ? 'or ' : '')/* LGEGMC-3167 add */
				...
				//LGEFR-640
					...
					if($priceArea.length!=0) {
						...
						// LGECZ-424 Start
						const $obsLowestPriceArea = $template.find('.lowest-price');
						if(p.obsLowestPrice=='Y' && $obsLowestPriceArea.length > 0) {
							$template.find('.model-buy').addClass('has-lowPrice');
						}else{
							$obsLowestPriceArea.filter('.lowest-price').remove();
						}
						// LGECZ-424 End
					}
					...
					if (p.afterPay <= 0 || p.obsInventoryFlag != 'Y') {
						$template.find('.price-vip-Installment > a.afterpay-installment').remove();
					}
					...
					if (!!p.limitSaleUseFlag && p.limitSaleUseFlag == 'Y' && !!p.obsLimitSale && p.obsLimitSale == 'Y') {
						$template.find('.price-vip-Installment .price-vip').text(productMessages.limitSaleTitle);
					}
				...
				ajax.call(url, param, 'json', function(d){
					...
					if(d.status == "success" && data) {
						...
						if($(filter2.result.querySelector('.promotion-wrapper')).length==0 && (data.productList.length > 0 || data.multiCategorys.length == 1)) {
							...
							if(isExpander) { // mobile append list
								...
								if(fastViewType == "S") {
									...
									$('.btn-fast').on('click' ,function(){
										...
										var resultCount = $('.result-info .total1 .result-number').text();
										if(parseInt(resultCount) > 0){
											var plpBox = $('.product-list-box');
											if($(plpBox.find('.no-content')).css("display") != "none") {
											...
											var resultCount2 = $('.result-text .total1 .result-number');
							...
							//PJTCURATION-8_2 Start
							if($(".GPC0134").length > 0 && $("input[name='resultViewType']").val() == "S") {
						...
						/* PJTCURATION-8 Start */
						if(data.exampleProductList && data.exampleProductList != null && data.exampleProductList.length > 0){
							var $target = $('.GPC0134 .result-sub-aria .GPC0004');
							html = filter2.createProductItemCuration(data.exampleProductList, d.data[0].productMessages);
							...
							if(data.siblingAjaxUrl && data.siblingAjaxUrl != null && $target.attr('data-model-url') == ''){
		...
		plp2 = {
				...
				addEvent: function(){
					...
					$(plp2.el.list).on({
						click: function(e){
							e.preventDefault();
							if(typeof ePrivacyCookies=='undefined' || ePrivacyCookies.get('LGCOM_IMPROVEMENTS')  ||COUNTRY_CODE.toLowerCase()=='uk' || COUNTRY_CODE.toLowerCase()=='es') { //LGEES-481
								var page;
								if($(e.currentTarget).closest('.expander')[0]) { // lead more button
									page = e.currentTarget.value;
