$(document).ready(function() {
	...
	var compareComponent = {
		...
		init: function(){
			...
			this.markup.membership = compareComponent.$el.find('template.product-member-price').clone().html(); //PJTMEMBERSHIP-11 add
			compareComponent.$el.find('template.empty-item').remove();
			compareComponent.$el.find('template.product-item').remove();
			//compareComponent.$el.find('template.product-member-price').remove(); //PJTMEMBERSHIP-11 add
		...
		switcher: function(e){
			...
			if(COUNTRY_CODE.toLowerCase() == 'cn'){
				if(!!$(this).closest('.GPC0009').length){
					baiduTrackEvent('add_to_compare_click', $(this).closest('.GPC0009'), 'PDP');
				}else if(!!$(this).closest('.GPC0007').length){
					baiduTrackEvent('add_to_compare_click', $(this).closest('li'), 'PLP');
				}
				
			}
			/* LGECN-214 End */
			
			if(typeof ePrivacyCookies=='undefined' || ePrivacyCookies.get('LGCOM_IMPROVEMENTS') || COUNTRY_CODE.toLowerCase()=='de' || COUNTRY_CODE.toLowerCase()=='it') { //LGEDE-926,LGEIS-1124
		...
		slick: {
			...
			switchLayer: function(open, count){
				...
				if(open) {
					...
					/* LGECN-214 Start */
					if(COUNTRY_CODE.toLowerCase() == 'cn' && typeof _hmt != 'undefined' && (lgCggroupName == 'pdp' || lgCggroupName == 'super-category' || lgCggroupName == 'category' || lgCggroupName == 'sub-category')){
						$(compareComponent.compareBtn).on('click',function(){
							var compare_quantity = $(this).closest('.GPC0022').find('.count:eq(0)').text();
							console.log('[Baidu trackEvent] [add_to_compare_submit] ', ', compare_quantity : '+ compare_quantity);
							_hmt.push(['_trackCustomEvent', 'add_to_compare_submit', {'compare_quantity': compare_quantity}]);
						});
					}
					/* LGECN-214 End */					
				}else {
					...
					compareComponent.$el.off('transitionend webkitTransitionEnd oTransitionEnd').on({
						'transitionend webkitTransitionEnd oTransitionEnd': function(){
							if(!compareComponent.$el.hasClass('showing')) {
								...
								$('body').removeClass('compare-sticky-showing');
								if($(".GPC0022").hasClass('compare-min') == false){
									$('body').removeClass('compare-sticky-min');
								}
							}
						}
			...
			fixBtn: function() {
				var cp22 = $(".GPC0022");
				// LGECL-326 Start
				var ukBtn = $(".intercom-lightweight-app, #intercom-container, .GPC0076, .epromotor-widget-area, ommi-widget, #callbell-iframe, .floating-whatsapp, #chat-widget-container");
				// LGECL-326 End
				var cp11 = $(".GPC0011");
				if(ukBtn.length > 0){
					ukBtn.addClass("cp22");
					if(cp22.hasClass("active")){
						if(cp22.hasClass("showing") && cp22.hasClass("compare-min") !== true){
							ukBtn.addClass("gang-hide");
						} else if(cp22.hasClass("showing") !== true && cp22.hasClass("compare-min") !== true){
							ukBtn.removeClass("gang-hide");
							ukBtn.removeClass('cp22');
						} else if(cp22.hasClass("showing") !== true && cp22.hasClass("compare-min") == true){
							ukBtn.removeClass("gang-hide");
							ukBtn.removeClass('cp22');
						}else{
				...
				if(cp11.length > 0){
					...
					ukBtn.removeClass("cp11")
				}
			},
			ommiShadowUi: function() {
				// shadowRoot stylesheet add
				if(document.querySelector('ommi-widget') !== null){
					var ommiShadow = document.querySelector('ommi-widget').shadowRoot;
					var linkElem = document.createElement('link');
					linkElem.setAttribute('rel', 'stylesheet');
					linkElem.setAttribute('href', '/lg5-common-gp/css/default.min.css');
					ommiShadow.appendChild(linkElem);
					$("ommi-widget").addClass('ready');
				}
			},
		},
