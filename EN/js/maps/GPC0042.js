$(document).ready(function() {
    ...
    $(".GPC0042").find('.inside-component .component-wrap').wrap('<div class="inside-inner"></div>');
    var countryCode = document.getElementsByTagName("html")[0].getAttribute("data-countrycode"); // LGEAE-221
	var component_01 = function(){
		...
		p.eventOn = function(){	
			...
			this.binderElem.$open.on("click", function(e){
				...
				$(this).closest(".list-btn-area").parents().find(".more-content").addClass("open").slideDown(400);

				// find scroll line
				var tg = $(this).closest(".component"),
					adjustOption = ($(".GPC0011").length>0) ? Math.round($(".GPC0011").outerHeight()) : 0,
			...
			this.binderElem.$close.on("click", function(e){
				...
				if(tg.find(".folding-slider").length > 0){
					...
					p.unSlider(sl);
					sl.parents('.GPC0042').find('.list-btn-area a.toggle').removeClass('show').focus();
				} else{
			...
			this.binderElem.$toggle.on("click", function(e){
				...
				if($(this).hasClass('show')){
					$(this).closest(".component").children(".desc").addClass("bg-gray");
					$(this).closest(".list-btn-area").parents(".cont-wrap").find(".more-content").addClass("open").slideDown(400);
					var tg = $(this).closest(".component");
					var sl = $(tg).find(".folding-slider");
					setTimeout(function(){
						p.slider(sl);
					...
					$(this).closest(".component").children(".desc").removeClass("bg-gray");
					$(this).closest(".list-btn-area").parents(".cont-wrap").find(".more-content").removeClass("open").slideUp(400);
					var tg = $(this).closest(".component");
					var sl = $(tg).find(".folding-slider");
					p.unSlider(sl);
				}
		...
		p.addAriaDescribedby = function(){
			...
			$('.GPC0042').each(function(){
				var $target;
				if($(this).find('.cont-head .title') && !$(this).find('.cont-head .title').is(':empty')) {
					$target = $(this).find('.cont-head .title');
				}
				if($target) {
					$target.attr('id', 'waGPC0042_'+waNumber);
					$(this).find('.cont-head .btn-area-wrap a.btn').attr('aria-describedby', 'waGPC0042_'+waNumber);
					$(this).find('.cont-head .btn-area-wrap a.link-text').attr('aria-describedby', 'waGPC0042_'+waNumber);
					waNumber++;
				}

				$(this).find('.list-container .list').each(function() {
					var $target;
					...
					if($target) {
						$target.attr('id', 'waGPC0042_'+waNumber);
						$(this).find('a.btn').attr('aria-describedby', 'waGPC0042_'+waNumber);
						waNumber++;
					}
				});
			});
		...
		p.addAACode = function(){ 
			...
			if(aaTagUseLocation.indexOf("promotion-offers-uae/sign-up") > -1){
				$(' .cont-wrap .visual-area a').attr('adobe-click','uae-sign-up-campaign_start');
	...
	/* LGEGMC-2177 End */
	$('.GPC0042').find(".btn-area-wrap a").on("click", function (e) {
		var $this = $(this);
		target="_blank"
